...
{
	PIXEL_DEBUG_PRINT("In square pixel 1.");
	...
	PIXEL_DEBUG_PRINT("Start point: ");
	PIXEL_DEBUG_PRINT(startPointA);
	PIXEL_DEBUG_PRINT(COMMA);
	PIXEL_DEBUG_PRINT(startPointB);
	PIXEL_DEBUG_PRINT(". end point: ");
	PIXEL_DEBUG_PRINT(endPointA);
	PIXEL_DEBUG_PRINT(COMMA);
	PIXEL_DEBUG_PRINT(endPointB);
	PIXEL_DEBUG_PRINTLN(".");

	PIXEL_DEBUG_PRINT(F("Max waves with this pen and gridsize: "));
	PIXEL_DEBUG_PRINT(maxWavesForGridAndPen);
	PIXEL_DEBUG_PRINT(F(", input density: "));
	PIXEL_DEBUG_PRINT(density);
	PIXEL_DEBUG_PRINT(F(", oOut number of waves: "));
	PIXEL_DEBUG_PRINTLN(noOfWaves);
...
//
	PIXEL_DEBUG_PRINT(F("TargetA: "));
	PIXEL_DEBUG_PRINT(targetA);
	PIXEL_DEBUG_PRINT(F(", targetB: "));
	PIXEL_DEBUG_PRINT(targetB);
	PIXEL_DEBUG_PRINT(F(". SourceA: "));
	PIXEL_DEBUG_PRINT(sourceA);
	PIXEL_DEBUG_PRINT(F(", sourceB: "));
	PIXEL_DEBUG_PRINT(sourceB);
	PIXEL_DEBUG_PRINTLN(F("."));
	if (targetA < sourceA && targetB < sourceA)
	{
		PIXEL_DEBUG_PRINTLN(F("calculated NW"));
		dir = DIR_NW;
	}
	else if (targetA > sourceA && targetB > sourceB)
	{
		PIXEL_DEBUG_PRINTLN(F("calculated SE"));
		dir = DIR_SE;
	}
	else if (targetA<sourceA && targetB>sourceB)
	{
		PIXEL_DEBUG_PRINTLN(F("calculated SW"));
		dir = DIR_SW;
	}
	else if (targetA > sourceA && targetB < sourceB)
	{
		PIXEL_DEBUG_PRINTLN(F("calculated NE"));
		dir = DIR_NE;
	}
	else if (targetA == sourceA && targetB < sourceB)
	{
		PIXEL_DEBUG_PRINTLN(F("calc NE"));
		dir = DIR_NE;
	}
	else if (targetA == sourceA && targetB > sourceB)
	{
		PIXEL_DEBUG_PRINTLN(F("calc SW"));
		dir = DIR_SW;
	}
	else if (targetA < sourceA && targetB == sourceB)
	{
		PIXEL_DEBUG_PRINTLN(F("calc NW"));
		dir = DIR_NW;
	}
	else if (targetA > sourceA && targetB == sourceB)
	{

		PIXEL_DEBUG_PRINTLN(F("calc SE"));
		dir = DIR_SE;
	}
	else
	{
		PIXEL_DEBUG_PRINTLN("Not calculated - default SE");
	}
	return dir;
}
...
int pixel_minSegmentSizeForPen(float penSize)
{
	...
	PIXEL_DEBUG_PRINT(F("Min segment size for penSize "));
	PIXEL_DEBUG_PRINT(penSize);
	PIXEL_DEBUG_PRINT(F(": "));
	PIXEL_DEBUG_PRINT(minSegSize);
	PIXEL_DEBUG_PRINT(F(" steps."));
	PIXEL_DEBUG_PRINTLN();
	return minSegSize;
}
...
int pixel_maxDensity(float penSize, int rowSize)
{
	...
	PIXEL_DEBUG_PRINT(MSG_D_STR);
	PIXEL_DEBUG_PRINT(F("G,D,rowsize in mm: "));
	PIXEL_DEBUG_PRINT(rowSizeInMM);
	PIXEL_DEBUG_PRINT(F(", mmPerStep: "));
	PIXEL_DEBUG_PRINT(mmPerStep);
	PIXEL_DEBUG_PRINT(F(", so rowsize in steps: "));
	PIXEL_DEBUG_PRINTLN(rowSize);
...
int pixel_scaleDensity(int inDens, int inMax, int outMax)
{
	...
	PIXEL_DEBUG_PRINT(F("inDens:"));
	PIXEL_DEBUG_PRINT(inDens);
	PIXEL_DEBUG_PRINT(F(", inMax:"));
	PIXEL_DEBUG_PRINT(inMax);
	PIXEL_DEBUG_PRINT(F(", outMax:"));
	PIXEL_DEBUG_PRINT(outMax);
	PIXEL_DEBUG_PRINT(F(", reduced:"));
	PIXEL_DEBUG_PRINTLN(reducedDens);
...
void pixel_drawWavePixel(int length, int width, int density, byte drawDirection, byte shape)
{
	...
	if (density > 0)
	{
		...
		PIXEL_DEBUG_PRINT("Basic seg length:");
		PIXEL_DEBUG_PRINT(basicSegLength);
		PIXEL_DEBUG_PRINT(", basic seg remainder:");
		PIXEL_DEBUG_PRINT(basicSegRemainder);
		PIXEL_DEBUG_PRINT(", remainder per seg");
		PIXEL_DEBUG_PRINTLN(remainderPerSegment);
		for (int i = 0; i <= density; i++)
		{
...
{
	pixel_drawWavePixel(length, width, density, drawDirection, SQUARE_SHAPE);
}
...
void pixel_drawWaveAlongAxis(int waveAmplitude, int waveLength, int totalWaves, int waveNo, byte dir, byte shape)
{
	...
	if (waveNo == 0)
	{
		...
		PIXEL_DEBUG_PRINT("First wave half");
		if (lastWaveWasTop)
...
void pixel_testPenWidth()
{
	...
	for (float pw = startWidth; pw <= endWidth; pw += incSize)
	{
		...
		PIXEL_DEBUG_PRINT(F("Penwidth test "));
		PIXEL_DEBUG_PRINT(iterations);
		PIXEL_DEBUG_PRINT(F(", pen width: "));
		PIXEL_DEBUG_PRINT(penWidth);
		PIXEL_DEBUG_PRINT(F(", max density: "));
		PIXEL_DEBUG_PRINTLN(maxDens);
